...
function showCountDownAndEnableShoot(time, startTime) {
    setInterval(function () {
        var remainingTime = parseInt((startTime - time) / 1000);
        ...
        // document.getElementById("timer-container").innerHTML = innerText;
        if(days > 0) {
        ...
        if (time >= startTime) {
            $('#shoot-button').attr('disabled', false);
            $('#shoot-button').text('SHOOT');
    ...
    .then(r => {
        ...
        fetch('/config/getsettings')
            ...
            .then(r => {
                ...
                if (time < startTime) {
                    $('#shoot-button').attr('disabled', true);
                    $('#shoot-button').text('Coming Soon');
                    $('#timer-container').show();
                    showCountDownAndEnableShoot(time, startTime);
                } else {
                    $('#timer-container').hide();
                }
            })
